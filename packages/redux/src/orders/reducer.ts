...
import type { LogoutAction } from '../users/types';
import type {
  OrderItemEntity,
...
export const entitiesMapper = {
  ...
  [actionTypes.RESET_ORDERS]: (state: StoreState['entities']) => {
    if (!state) {
      return state;
    }
 ...
 * @returns New state.
 */
const ordersReducer = (state: T.OrdersState, action: LogoutAction) => {
